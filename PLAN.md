# Plan: PromptPacker VS Code Extension

## Context
Developers who use PromptPacker to prepare AI prompts currently have to open a browser, drag files in, and copy output. A VS Code extension eliminates that friction: right-click files or folders in the Explorer → "Add to PromptPacker" → panel opens with the combined prompt ready to copy. All the core logic (file filtering, gitignore parsing, output formatting) already exists in the web app and can be ported with minimal changes.
...
```json
{
  ...
  "activationEvents": [
    "onCommand:promptpacker.addToPromptPacker",
    "onCommand:promptpacker.openPanel",
    "onCommand:promptpacker.clearAll"
  ],
  "contributes": {
    "commands": [
      { "command": "promptpacker.addToPromptPacker", "title": "Add to PromptPacker" },
      { "command": "promptpacker.openPanel",    "title": "PromptPacker: Open Panel" },
      { "command": "promptpacker.clearAll",     "title": "PromptPacker: Clear All Files" }
    ],
    "menus": {
      "explorer/context": [
        {
          "command": "promptpacker.addToPromptPacker",
          "group": "navigation@99"
        }
      ]
    }
...
12. `statusBar.ts` — `$(files) PromptPacker: N files`, clicking opens panel
...
**Symptom:** "Add to PromptPacker" never appeared in the Explorer context menu for file right-clicks.
...
3. Right-click a single file → "Add to PromptPacker" appears in context menu → panel opens with file listed
